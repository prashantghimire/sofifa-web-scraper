...
async function getPlayerDetailsRow(url) {
    ...
    function getPlayerAttributes() {
        // 7. player_attributes
        const player_attr_grid1 = $(grids[2]).find('.col p').each((i, e) => $(e).find('span.plus').remove()).map((i, el) => $(el).text()).get();
        const player_attr_grid2 = $(grids[3]).find('.col p').each((i, e) => $(e).find('span.plus').remove()).map((i, el) => $(el).text()).get();
...
(async function () {
    const row = await getPlayerDetailsRow('https://sofifa.com/player/278046/pau-cubarsi-paredes/240033/');
